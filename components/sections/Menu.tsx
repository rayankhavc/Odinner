import { menu, viandes, sauces, supplements, extras, site } from "@/lib/data";
...
export default function Menu() {
  return (
    <section id="carte" className="on-paper bg-paper text-ink">
      <div className="wrap pt-20 sm:pt-24">
        <Reveal>
          <div className="max-w-2xl">
            <p className="eyebrow text-brand-ink">La carte</p>
            <h2 className="h2 mt-5 text-balance">Toute la carte, prix compris.</h2>
            <p className="lede mt-5 text-pretty text-ink/60">
              Les sandwichs et les assiettes partent avec crudités et frites.
      ...
      {/* Le rail collant reste hors de tout parent anime : une transformation
          CSS sur un ancetre casserait le position: sticky. */}
                  ...
                  {category.items.map((item) => (
                    <li key={item.name}>
                      <div className="flex items-baseline justify-between gap-2 sm:justify-start">
                        <span
                          className={`font-semibold ${item.signature ? "text-brand-ink" : ""}`}
                        >
                          {item.name}
                          {item.veggie ? (
                            <LeafIcon
      ...
      {/* Ce qu'on choisit soi-meme : viandes, sauces, supplements */}
      <div className="wrap grid gap-4 border-t border-paper-line py-12 md:grid-cols-3">
        ...
        <Reveal delay={0.16} className="rounded-2xl bg-paper-soft p-6">
          <h3 className="h3">Suppléments et formules</h3>
          ...
          <div className="flex flex-col items-center gap-5 rounded-2xl bg-ink px-6 py-9 text-center sm:flex-row sm:justify-between sm:text-left">
            <div>
              ...
              <p className="mt-2 text-[0.9375rem] text-bone/55">
                On prépare, vous passez récupérer. Ou vous vous installez en salle.
              </p>
            </div>
